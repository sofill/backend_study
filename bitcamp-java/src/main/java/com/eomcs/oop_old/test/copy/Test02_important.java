...
public class Test02_important {
  public static void main(String[] agrs) {
    ...
    // 메모리를 만들 때는 일반적인 방식으로는 만들 수 없고
    // 반드시 new 명령을 사용해야 한다.
    // 준비된 메모리를 사용하기 위해서는 그 메모리의 주소를 변수에 보관해야 한다.
    // 이렇게 class 설계도로 만든 메모리에 주소를 저장하는 변수를
    ...
    // - 다른 설계도로 만든 메모리의 주소는 저장할 수 없다.
    // - Member 설계도로 만든 메모리는 이 변수를 통해 접근할 수 있다.
    //
    // ② new Member()
    // - Member 설계도에 따라 메모리를 준비시키는 명령이다.
    // - 이 명령을 통해 생성된(=사용할 수 있도록 RAM이 준비해 준) 메모리를
    // "인스턴스(instance)"라고 한다.
    // 보통은 "객체(object)"라고 부른다.

    // ③ 사용법
    // - 레퍼런스를 통해 해당 메모리에 접근할 수 있다.
    m1.no = 100;
    m1.name = "홍길동";
    m1.birthYear = 2001;
    m1.height = 180.5f;
