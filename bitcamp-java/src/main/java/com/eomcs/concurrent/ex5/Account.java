...
public class Account {
  ...
  // 어떻게? 메서드 앞에 synchronized를 붙인다. (sychronized는 동기화 문법)
  // => 이 메서드처럼 여러 스레드가 접근했을 때 문제가 발생하는 critical section이 아닌데도
  // synchronized 를 사용한다면 실행 속도가 떨어진다. (성능이 떨어짐)
  // 참고*
  // => 여러 스레드가 동시에 실행해도 문제가 없는 코드블럭을
  //    "스레드 안전(thread safe)"이라 부른다. (synchronized 안 붙여도 됨)
  //
  synchronized public long withdraw(long money) {
    ...
    //
    long b = this.balance; // 자바의 기본단위는 4바이트(32비트)이기 때문에 long으로 해줘야 CPU를 안 뺏긴다.
