...
public class Exam0120 {
  ...
  public static void main(String[] args) {
    ...
    // => 그러면 JVM은 이 스레드에 들어 있는 코드와 다음에 진행하는 코드를
    //    왔다갔다 하면서 처리할 것이다.
    new MyThread().start();
...
//
// ## 멀티태스킹 (multi-tasking)
//   - 한 개의 CPU 가 여러 코드를 동시에 실행하는 것.
//   - 실제로는 일정한 시간을 쪼개 이 코드와 저 코드를 왔다갔다 하면서 실행한다.
//   - 그럼에도 불구하고 외부에서 봤을 때는 명령어가 동시에 실행되는 것처럼 보인다.
//   - 왜? CPU 속도가 워낙 빠르기 때문이다.
//
// ## CPU 의 실행 시간을 쪼개서 배분하는 정책이 필요!
//  => "CPU Scheduling" 또는 프로세스 스케쥴링이라 함.
//    - CPU 의 실행 시간을 쪼개서 코드를 실행하는 방법이다.
//
//    1) Round-Robin 방식
...
//       - 문제점 : 우선 순위가 낮은 프로그램인 경우 CPU 시간을 배정 받지 못하는 문제가 발생함.
//                  그래서 몇 년이 지나도록 실행되지 않는 경우가 나타남.
//       - 해결책 ?
//         CPU 시간을 배정 받지 못할 때마다 우선 순위를 높여서 언젠가는 실행되게 만들도록 함.
//         이런 방식을 "에이징(Aging) 기법" 이라 부른다.
//
// ## 멀티 태스킹을 구현하는 방법
//  1) 멀티 프로세싱 (프로세스 복제 방식). fork()
//   - 프로세스(실행 중인 프로그램)를 분기하여 동시에 작업을 실행시키는 방법.
//     그리고 분기된 프로세스를 실행시켜서 작업을 동시에 진행하게 한다.
//   - 장점 : 분기하기가 쉽다. (fork()호출). 즉 구현하기(프로그래밍)가 쉽다.
//   - 단점 : 프로세스를 그대로 복제하기 때문에 프로세스가 사용하는 메모리도 그대로 복제된다.
//            한마디로 메모리의 낭비가 심하다.
//            또, 복제된 프로세스는 독립적이기 때문에 실행 종료할 때도 일일이 종료해야 한다.
//
//  2) 멀티 스레딩
//   - 특정 코드만 분리하여 실행시키는 방법.
//     따라서 프로세스가 사용하는 메모리를 공유한다.
//   - 장점 : 프로세스의 힙 메모리를 공유하기 때문에 메모리 낭비가 비교적 적다.
//            모든 스레드는 프로세스에 종속되기 때문에 프로세스를 종료하면
//            스레드도 자동 종료된다. (관리 용이)
//   - 단점 : 프로세스 복제 방식에 비해 코드 구현이 복잡하다.
//
// ## 컨텍스트 스위칭(context switching)   * context : 실행환경 정보
...
// ## 스레드 생성
//   - 새 실을 만든다는 것이다.
//   - 즉 새 실행 흐름을 시작하겠다는 의미이다.
//   - CPU 는 스레드를 프로세스와 마찬가지로 동일한 자격을 부여하여
//     스케줄링에 참여시킨다.
//   - 즉 프로세스에 종속된 스레드라고 취급하여
//     한 프로세스에 부여된 실행 시간을 다시 쪼개 스레드를 실행한다는 것이 아니다.
//     단독적인 프로세스처럼 '동일한 실행 시간을 부여'하는 것이다!
//
