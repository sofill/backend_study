...
public class Exam0360 {
  ...
  public static void main(final String[] args) {
    ...
    final Runner r = obj.createRunner("신애");
  ...
  Runner createRunner(final String name) {
    ...
    class A implements Runner {
      ...
      public void run() {
        ...
        //
        // 위의 문제를 방지하기 위해 컴파일러는
        // 바깥 메서드의 로컬 변수 값을 저장할 필드를 클래스에 추가한다.
        // 또한 로컬 변수의 값을 받는 생성자도 만든다.
        // 그래서 run() 메서드를 호출하는 시점에는
        // A 로컬 객체에 name 변수의 값이 들어 있기 때문에
        // 바깥 메서드의 로컬 변수를 사용할 수 있는 것이다.
        //
        // => 다음 코드의 name은 createRunner() 메서드의 파라미터가 아니다.
        // => A 인스턴스가 생성될 때 값을 따로 복제한 필드를 가리키는 것이다.
        // 그래서 createRunner() 메서드 호출이 끝나더라도 name 값을 사용할 수 있는 것이다.
        //
