...
public abstract class Car {
    ...
    // => 왜? 추상 메서드가 있다는 것은 해당 메서드를 실행할 수 없다는 뜻이고
    //       실행할 수 없는 메서드는 갖는 클래스는 
    //       인스턴스를 생성해서는 안되기 때문에
    //       추상메서드를 갖는 클래스는 반드시 추상클래스여야 한다.
    //       일반 클래스는 추상메서드를 가질 수 없다.
    // 
    public abstract void run(); //이게 추상메서드다. 추성메서드는 구현하면 안되고 선언만..!
    // 중괄호 적지 않는다. 중괄호 적으면 구현.
    //abstract public void run(); 으로 해도 가능은 하지만 보통은 위처럼 한다.
