...
public class Exam0310 {
  public static void main(String[] args) {
    ...
    // 가비지 컬렉터(garbage collector)의 실행은?
    //  1. 메모리 부족할 때★
    //  2. CPU가 한가할 때 -개발자는 알 수가 없다.(모니터링 툴이 있기는 하지만 일반 개인은 사용을 잘 안하므로)
    //  3. System.gc()를 호출하여 가비지 실행을 "요청"할 때 (강제 x)
    //     ★물론 이 경우에 바로 실행하는 것이 아니라
    //      빠른 시간 내에 실행할 것을 종용하는 것이다.
    //      JVM이 판단하기에 아직 실행할 상황이 아니라 판단하면 무시하게 될 수도 있다. 
