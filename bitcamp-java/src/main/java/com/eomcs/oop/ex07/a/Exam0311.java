...
class Score4 {
  ...
  // => sum, aver 필드 같은 경우
  //    값을 조회만 해야하기 때문에 '게터'만 있다.
  // => 이렇게 필드에 대해서 항상 게터/세터를 모두 만드는 것은 아니다.
  // 
  // 용어
  // => name, kor, eng, math, sum, aver 는 "필드(field)"라 부른다.
  // => getXxx()/setXxx()는 "프로퍼티(property)"라 부른다.
  // => 주의 : 필드를 프로퍼티라 부르는 것이 아니다!
  //         게터/세터 를 프로퍼티라고 부르는 것이다!
  ...
  public void setKor(int kor) { 
    this.kor = kor;
    this.compute(); //이렇게 넣으면 된다.(바꿀 때마다 계산 다시 하도록) 
  }
  public int getKor() {
    return this.kor;
  }
...
public class Exam0311 {
  public static void main(String[] args) {
    ...
    s1.setName("홍길동"); // Name은 이렇게 안해도 되는데 프로그래밍의 "일관성"을 위해서 세터게터로 바꿔 준다. 
    s1.setKor(100);
    s1.setEng(90);
    s1.setMath(80);
