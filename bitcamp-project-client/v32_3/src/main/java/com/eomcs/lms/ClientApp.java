...
public class ClientApp {
  ...
  public static void main(String[] args) throws Exception {
    ...
    try {
      ...
      System.out.println("서버 주소 또는 포트 번호가 유효하지 않아요!");
      keyScan.close();
      return;
    }
    ...
    try (
        ...
        // 서버와 연결
        Socket socket = new Socket(serverAddr, port); // 로컬 주소, 포트번호
      ...
      // 서버에 메시지를 전송한다.
      // 주의 => 서버가 메시지를 받을 때까지 리턴하지 않는다.
      //      => 즉, blocking 방식으로 동작한다.
      out.println(sendMsg);
      System.out.println("서버에 보낼 메시지를 전송하였음!");
      // 서버가 응답한 메시지를 수신한다.
      // 주의 => 서버로부터 한 줄의 메시지를 받을 때까지 리턴하지 않는다.
      //      => 즉, blocking 방식으로 동작한다는 것을 잘 기억할 것.
