...
- DBMS 에 사용자와 데이터베이스를 추가할 수 있다.
- 테이블 생성과 예제 데이터를 입력할 수 있다.
- JDBC API의 목적을 이해한다.  
	=> 호출 규칙을 통일하는 것!
	=> 즉, DBMS 와의 통신을 담당하는 proxy 객체의 사용 규칙을 통일하는 것!
- JDBC Driver 의 의미를 이해한다.
	- JDBC API 규칙에 따라 정의한 클래스들. ( = 라이브러리)
- JDBC API 를 활용하여 DBMS에 데이터를 입력, 조회, 변경, 삭제할 수 있다.
...
### 훈련1 : 애플리케이션에서 사용할 사용자와 데이터베이스를 MariaDB 에 추가한다.
...
bitcamp-study    (Git 저장소)
    /bitcamp-docs  
        /db
           /project0-ddl.sql  (테이블 정의 SQL 문이 들어 있는 파일)
```
