...
public class DataLoaderListener implements ApplicationContextListener {
  ...
  public void contextInitialized(Map<String,Object> context) {
    ...
    // 이 메서드에서는 애플리케이션이 작업한 데이터를 시작하는 일을 한다.
    loadLessonData();
    loadMemberData();
    loadBoardData();

    // 데이커가 저장되어 있는 List 객체를
    // 이 메서드를 호출한 쪽(App클래스) 에서 사용할 수 있도록
    // Map 객체에 담아둔다.
  ...
  public void contextDestroyed(Map<String,Object> context) {
    ...
    // 애플리케이션이 시작되면 이 메서드가 종료될 것이고,
    // 이 메서드에서는 애플리케이션이 작업한 데이터를 종료하는 일을 한다.
    saveLessonData();
    saveMemberData();
    saveBoardData();
  }
