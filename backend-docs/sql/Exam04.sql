# FK(Foreign Key)
- 다른 테이블의 PK를 참조하는 컬럼이다. 다른 테이블의 식별번호를 가리키는 key 

* key - 데이터를 구분할 때 사용하는 값 (ex. 전화, 이메일, 이름-전화, 이름-이메일)
* primary key 주키, 기본 키 - 한 번 값이 결정되어 사용한 후에는 절대 못 바꾼다. 
* foreign key 외래 키 - 
* candidate key 후보키 = 최소키(최소한으로 줄이고 추려낸 것) 
               ex. 전화, 이메일 ----선정----> 이메일(주키-PK)
                                ----선정----> 전화 (대안키-Alternate key) ---> Unique 컬럼
* alternate key 대안키 - 중복은 안됨. 변경은 가능 ----(보통)선정-----> Unique 컬럼
* artificial key 인공키 - pk의 값은 변경할 수 없으므로 그에 대한 해결책으로 나온 것이 인공키.
                        주로 정수 번호 사용.
...
해결책?!
- 다른 데이터를 참조하는 경우 해당 데이터의 존재 유무를 검사하도록 강제하는 방법.
