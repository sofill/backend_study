...
public class App {
  ...
  public static void main(String[] args) {
    ...
    // 단지 유지 보수를 좋게 하기 위해 ArrayList와 LinkedList의 
    // 공통 분모를 뽑아서 만든 클래스가 List이다. 
    // List 클래스는 실제 작업을 하는 클래스가 아니다. 
    // 그럼에도 불구하고 개발자가 아래와 같이 List 객체를 사용하려 한다면
    // 막을 수 없다.
    // => BoardHandler의 경우 작업을 제대로 수행하지 않을 것이다.
    // => 왜? List 클래스에 정`의된 메서드는 아무것도 하지 않는다.
    //List<Board> boardList = new List<>();
    // 
    // 해결책?
    // => 이렇게 generalization을 통해 만든 클래스의 경우
    //    서브 클래스에게 공통 분모를 물려주기 위한 용도로 사용된다.
    // => 이런 류의 클래스는 직접 인스턴스를 생성하지 못하도록 헤서
    //    인스턴스를 직접 사용하는 것을 막아야 한다.
    // => 이런 용도로 사용하는 문법이 "추상 클래스(abstract class)"이다.
    //
    // List 클래스(AbstractList)로 이름 변경함)를 추상 클래스로 만들면,
