...
public class Queue<E> extends LinkedList<E> implements Cloneable { //클론메서드 재정의 
  ...
  public E poll() {
    return this.remove(0); //값 꺼내는 것:remove로
  }
  ...
  public Queue clone() {
    try {
      ...
      // => 따라서 다음에 다시 복제를 수행하면
      //    원본 노드으 ㅣ링크 정보가 제거되었기 대문에 제대로 작업을 수행할 수 없다.
      // 
      // 해결책?
      ...
      System.out.println(ex);Queue.java
      return null;
    }
  }
   */
