...
public class Stack<E> implements Cloneable { //Cloneable:복제가능클래스라는 인식표같은 것
  ...
  public E pop() {
    if (this.empty()) // <== 아래 empty() 만들었으니 이것도 리팩토링함.
      return null;
    E value = this.elementData[--this.size];
    this.elementData[this.size] =  null; // 동영상참조
    return value;  
  }
  ...
  // Object.clone()의 'shallow copy'를 이용하여 스택 객체 복사하기
  ...
  public Stack clone() {
    ...
    } catch (CloneNotSupportedException ex) { //문제 발생하면 아래의 null로 대신하라는 명령
      // Object의 clone() 메서드는 
      // 복제가 허용된 객체에 대해서만 해당 인스턴스 변수를 복제한다.  
      // 복제가 허용되지 않은 객체에 대해서 클론을 호출하면 
      // CloneNotSupportedException 실행 오류가 발생한다.
      // 해결책?
      // => 해당 클래스의 객체를 복제할 수 있다고 표시하라. 
      // => 방법 : Cloneable 인터페이스를 지정
  ...
  public Stack<E> clone()  {
    ...
    //  => 배열만 복제하고 그 배열에 저장된 객체(ex.문자열)까지는 복제하지 않는다. 
